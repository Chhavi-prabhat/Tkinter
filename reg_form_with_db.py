...
def show_details():
    show=Tk()
    show.title("Details window")
    show.maxsize(width=600, height=400)
    show.minsize(width=600, height=400)
    conn= sqlite3.connect('Registration.db')
    c=conn.cursor()
    c.execute('''SELECT * FROM Registration''')
    records=c.fetchall()
    display_txt= Text(show, height=50, width=50)
    display_txt.pack()
    for record in records:
        display_txt.insert(END, f"Name: {record[0]}\nDOB: {record[1]}\nPhone: {record[2]}\n\n")
...
details_btn = Button(win, text='Show Details', bg="Yellow", fg="white", font="arial", bd=3, command=show_details)
details_btn.place(x=350, y=170)
create_table()
